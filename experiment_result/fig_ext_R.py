...
runs = 100
base = ['#029386','#f97306','#c2b709','#c20078','#75bbfd','#929591']
correspondence = ['EA','CT','ET','TT','UT','SQ']
addon = ['#0504aa']
...
section = gridspec.GridSpecFromSubplotSpec(59, 80, subplot_spec = all_scenarios[0])
ax = {}
h_pos=[0,20,40,60]

for ax_idx in range(len(scenarios)):
    '''import the data'''
    path = sys.path[0]+'\\ext_R_3m_per_wc_{}.xlsx'.format(scenarios[ax_idx])
    ...
    ax[ax_idx] = fig.add_subplot(section[2:22 , h_pos[ax_idx]:h_pos[ax_idx]+10])
    # set different range to create broken axis
    ax[ax_idx].set_ylim(bottom=0.3, top=1)
    ...
    ax[ax_idx].scatter(bottoms, np.zeros_like(bottoms)+0.35,marker="X",s=70,color='r',edgecolors='k',zorder=5)
    # ax[ax_idx].violinplot(sum.transpose(), positions=x_position, showmeans=True, )
    # ticks
    ax[ax_idx].set_yticks(np.arange(0.3,1.1,0.1))
    ...
    plt.setp(ax[ax_idx].get_xticklabels(),  ha='right', rotation=20, rotation_mode="anchor", fontsize=7.5)
    ...
    ax[ax_idx].legend(handles=legend_elements, fontsize=6, loc=loc_list[ax_idx], ncol=2)



    '''and the pie chart'''
    ax[ax_idx] = fig.add_subplot(section[3:11 , h_pos[ax_idx]+11:h_pos[ax_idx]+17])
    '''retrive the data'''
    name = []
    win_rate = []
    colors = []
    ...
    wedges,texts = ax[ax_idx].pie(win_rate, labels = label, labeldistance=1.3, colors=colors, startangle=90, wedgeprops=dict(ec='w'), textprops=dict(fontsize=7))
    ...
    '''and the pie chart'''
    ax[ax_idx] = fig.add_subplot(section[15:23 , h_pos[ax_idx]+11:h_pos[ax_idx]+17])
    '''retrive the data'''
    name = []
    win_rate = []
    colors = []
    ...
    label = [None if win_rate[i]<=2 and win_rate[i-1]<=3 else str(win_rate[i]) for i in range(no)]
    annotates = [str(win_rate[i]) if win_rate[i]<=2 and win_rate[i-1]<=3 else None for i in range(no)]
    wedges,texts = ax[ax_idx].pie(win_rate, explode=explode, labels = label, labeldistance=1.3, colors=colors, startangle=90, wedgeprops=dict(ec='w'), textprops=dict(fontsize=7))
    plt.setp(texts, ha='center', )
...
for ax_idx in range(len(scenarios)):
    '''import the data'''
    path = sys.path[0]+'\\ext_R_4m_per_wc_{}.xlsx'.format(scenarios[ax_idx])
    data_sum = load_workbook(path)['sum']
    data_before_win_rate = load_workbook(path)['before win rate']
    data_win_rate = load_workbook(path)['win rate']
    '''create the grids'''
    #ax[ax_idx] = fig.add_subplot(section[bound*(ax_idx%2):bound*(ax_idx%2+1) , 10*(ax_idx%2):10%(ax_idx%2+1)])
    ax[ax_idx] = fig.add_subplot(section[30:50 , h_pos[ax_idx]:h_pos[ax_idx]+10])
    # set different range to create broken axis
    ax[ax_idx].set_ylim(bottom=0.3, top=1)
    ax[ax_idx].set_xlim(left = -0.5, right=benchmark_no-0.5)
    '''retrive the data'''
    ...
    '''and the pie chart'''
    ax[ax_idx] = fig.add_subplot(section[31:39 , h_pos[ax_idx]+11:h_pos[ax_idx]+17])
    '''retrive the data'''
    name = []
    win_rate = []
    colors = []
    ...
    '''and the pie chart'''
    ax[ax_idx] = fig.add_subplot(section[43:51 , h_pos[ax_idx]+11:h_pos[ax_idx]+17])
    '''retrive the data'''
    name = []
    win_rate = []
    colors = []
...
'''common legend'''
ax[100] = fig.add_subplot(section[54:, :])
# common legend
...
for i in correspondence.copy():
    ...
    if i not in name:
        ...
        del base[idx]
base += addon
correspondence += [r'$\mathbf{DRL-RA}$']
...
ax[100].legend(handles=legend_elements, fontsize=8, loc=2, ncol=7)
ax[100].axis('off')
# annotations
ax[100].set_xlim(0,100)
ax[100].set_ylim(0,100)
sign_x = 55
ax[100].text(sign_x,90,'DRL(✖)',va='center',fontsize=9, ha='center')
ax[100].text(sign_x+5,90,'Result excluding DRL agents',va='center',fontsize=9)

ax[100].text(sign_x,50,'DRL(✔)',va='center',fontsize=9, ha='center')
ax[100].text(sign_x+5,50,'Result including DRL agents',va='center',fontsize=9)

ax[100].scatter(sign_x,10,marker="X",s=70,color='r',edgecolors='k')
ax[100].text(sign_x+5,10,'Lower-than-baseline performance',va='center',fontsize=9)

'''common title'''
ax[11] = fig.add_subplot(section[0:, :])
ax[11].set_title('(1) 3-machine per work centre test')
ax[11].axis('off')
ax[12] = fig.add_subplot(section[28:, :])
ax[12].set_title('(2) 4-machine per work centre test')
ax[12].axis('off')


fig.subplots_adjust(top=0.95, bottom=0.1, hspace=0.5)
fig.savefig(sys.path[0]+"/ext_R_result.png", dpi=600, bbox_inches='tight')
plt.show()
